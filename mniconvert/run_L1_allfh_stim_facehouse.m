...
for ii=1:size(Ss,2),
    ...
    % ----
    disp('Calculating L1 stimulus (i.e. face/house)')
    savedir = [Ss{ii} '_stim_facehouse']
    [stat, meSs] = rmdir(fullfile(datadir,savedir), 's')  
            %% Force removal of old runs 
    %meta_L1_factorial(datadir,savedir,'warfh.nii', ...
	%    ['fidl/nod_' NODS{ii} '_stim_facehouse.mat'], ...
	%    'rp_fh0.txt',1.5,29,1,'stim',2)
    meta_L1_nomovement(datadir,savedir,'warfh.nii', ...
	    ['fidl/nod_' NODS{ii} '_stim_facehouse.mat'], ...
	    1.5,29,1)
